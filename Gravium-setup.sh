#!/bin/bash
# Gravium Masternode Setup Script V1.3 for Ubuntu 16.04 LTS
# (c) 2018 by RUSH HOUR MINING for Gravium
#
# Script will attempt to autodetect primary public IP address
# and generate masternode private key unless specified in command line
#
# Usage:
# bash Gravium-setup.sh [Masternode_Private_Key]
#
# Example 1: Existing genkey created earlier is supplied
# bash Gravium-setup.sh 27dSmwq9CabKjo2L3UD1HvgBP3ygbn8HdNmFiGFoVbN1STcsypy
#
# Example 2: Script will generate a new genkey automatically
# bash Gravium-setup.sh
#
...
cp ~/GRVmasternodesetup/nodemon.sh /usr/local/bin
sudo chmod 711 /usr/local/bin/nodemon.sh

#Create datadir
if [ ! -f ~/.graviumcore/gravium.conf ]; then 
	sudo mkdir ~/.graviumcore
fi

echo -e "${YELLOW}Creating gravium.conf...${NC}"

# If genkey was not supplied in command line, we will generate private key on the fly
if [ -z $genkey ]; then
    cat <<EOF > ~/.graviumcore/gravium.conf
rpcuser=$rpcuser
rpcpassword=$rpcpassword
EOF

    sudo chmod 755 -R ~/.graviumcore/gravium.conf
    ...
    #Stopping daemon to create gravium.conf
    stop_daemon
    delay 30
fi
...
#Finally, starting gravium daemon with new gravium.conf
graviumd
delay 5

#Setting auto star cron job for graviumd
cronjob="@reboot sleep 30 && graviumd"
crontab -l > tempcron
...
for your Hot Wallet (the wallet with your gravium collateral funds):
======================================================================== \a"
...
To introduce your new masternode to the gravium network, you need to
...
Currently your masternode is syncing with the gravium network...
...
${YELLOW}cat ~/.graviumcore/gravium.conf${NC}
Here is your gravium.conf generated by this script:
-------------------------------------------------${YELLOW}"
cat ~/.graviumcore/gravium.conf
echo -e "${NC}-------------------------------------------------
NOTE: To edit gravium.conf, first stop the redend daemon,
then edit the reden.conf file and save it in nano: (Ctrl-X + Y + Enter),
then start the redend daemon back up:
to stop:   ${YELLOW}gravium-cli stop${NC}
to edit:   ${YELLOW}nano ~/.graviumcore/gravium.conf${NC}
to start:  ${YELLOW}graviumd${NC}
========================================================================
To view graviumd debug log showing all MN network activity in realtime:
${YELLOW}tail -f ~/.graviumcore/debug.log${NC}
========================================================================
...
========================================================================
Enjoy your gravium Masternode and thanks for using this setup script!
If you found it helpful, please donate gravium to:
GcNmie8X5Pqu8NCbLPwLka9nNNMVLnA9rs
